...
historian.Context = function (
   ...
   */
  this.xScaleUntransformed_ = d3
    .scaleUtc()
    .domain(
      this.xDomain_.map(function (unix) {
  ...
  if (this.location) {
    ...
    this.xAxis_.tickFormat(
      function (date) {
        ...
        // savings.
        var offsetMs =
          moment(date).tz(this.location).utcOffset() *
  ...
  var formattedInts = function (d) {
    ...
    return Number.isInteger(d)
      ? historian.color.valueFormatter(config.name, d, config.ticksShortForm)
          .value
      : "";
  }.bind(this);
  this.yAxis_.tickFormat(
    function (d) {
      var config = this.levelData_.getConfig();
   ...
   * @type {!d3.selection}
   */
  this.seriesLinesGroup = this.svg
    .append("g")
   ...
   * @type {!d3.selection}
   */
  this.svgChart = transformed.append("svg").attr("class", "svg-chart");
  ...
  var clip = this.svgChart.append("svg:clipPath").attr("id", "clip");
  ...
  this.clipRect_ = clip
    ...
    .attr(
      "height",
      this.visSize[historian.constants.HEIGHT] + historian.Context.MARGINS.TOP
    );
   ...
   * @type {!d3.selection}
   */
  this.svgClipped = this.svgChart.append("g").attr("clip-path", "url(#clip)");
   ...
   * @type {!d3.selection}
   */
  this.svgLevelBoxes = this.svgClipped.append("g").attr("class", "level-boxes");
  /**
   * Group for rendering series-level hovering boxes.
   * @type {!d3.selection}
   */
  this.svgSeriesLevel = this.svgClipped
    .append("g")
   ...
   * @type {!d3.selection}
   */
  this.svgBars = this.svgClipped.append("g").attr("class", "bars");
  /**
   * Group for rendering the level line.
   * @type {!d3.selection}
   */
  this.svgLevel = this.svgClipped.append("g").attr("class", "level");
  /**
   * Group for rendering the report taken line.
   * @type {!d3.selection}
   */
  this.svgLevelEventMarkers = transformed
    .append("g")
   ...
   * @type {!d3.selection}
   */
  this.svgLevelSummaries = this.svgClipped
    .append("g")
   ...
   * @type {!d3.selection}
   */
  this.svgLevelViewInfo = this.svgClipped
    .append("g")
  ...
  this.zoom = d3
    .zoom()
    .scaleExtent([1, 5120000])
    .translateExtent([
      [0, 0],
      [this.svgSize[historian.constants.WIDTH], 0],
    ])
    .on("zoom", zoomHandler);
  this.svgChart.call(this.zoom.bind(this)).on("wheel", function () {
...
historian.Context.prototype.renderYAxis = function () {
  ...
  var selected = this.container_
    .find(historian.constants.Elements.LEVEL_SELECT + " option:selected")
    .val();
  if (!selected) {
    return;
  }
  var yAxisXOffset =
    historian.Context.MARGINS.LEFT + this.visSize[historian.constants.WIDTH];
  this.svg
    .append("svg:g")
    .attr("class", "y axis")
    .attr(
      "transform",
      "translate(" + yAxisXOffset + ", " + historian.Context.MARGINS.TOP + ")"
    )
    .call(this.yAxis_);
};
...
historian.Context.prototype.renderAxes_ = function () {
  ...
  if (this.location != "") {
    ...
    var startTimeZone = historian.time.getTimeZoneShort(
      this.xDomain_[0],
      this.location
    );
    var endTimeZone = historian.time.getTimeZoneShort(
      this.xDomain_[1],
      this.location
    );
    var shortTimeZone =
...
historian.Context.prototype.redrawTicks_ = function () {
  var ticks = this.svg.select(".x.axis").selectAll(".tick").nodes();
