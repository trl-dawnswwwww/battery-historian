  ...
  if (apk) {
    ...
    if (apk.service && apk.service.length > 0) {
      ...
      apk.service.sort(function (a, b) {
        return (
          b.start_time_msec - a.start_time_msec ||
...
historian.appstats.getControllerActivityDetails = function (name, controller) {
  ...
  rows.push([
    ...
    controller.idle_time_msec
      ? historian.time.formatDuration(controller.idle_time_msec)
      : "0s",
  ]);
  var tx_time_msec = 0;
  ...
  rows.push([
    name + " transfer time",
    goog.string.subs(
      "%s total (%s receiving, %s transmitting)",
 ...
 */
historian.appstats.displayAppNetworkInfo = function (
  network,
  wifi,
  wifi_controller,
  modem_controller,
  ...
  if (
    ...
    if (network) {
      ...
      bodyRows.push([
        ...
        goog.string.subs(
          "%s total (%s received, %s transmitted)",
      ...
      bodyRows.push([
        ...
        goog.string.subs(
          ...
          network.mobile_packets_rx,
          network.mobile_packets_tx
        ),
      ]);
      bodyRows.push([
        ...
        goog.string.subs(
          ...
          network.mobile_packets_bg_rx,
          network.mobile_packets_bg_tx
        ),
      ]);
      bodyRows.push([
        ...
        goog.string.subs(
          ...
          network.wifi_packets_rx,
          network.wifi_packets_tx
        ),
      ]);
      bodyRows.push([
        ...
        goog.string.subs(
          ...
          network.wifi_packets_bg_rx,
          network.wifi_packets_bg_tx
        ),
      ]);
      var row = [
        "Mobile active time",
        ...
        bodyRows.push([
          {
            ...
            title:
              "Scan time split among apps performing wifi scans at the " +
              "same time.",
          },
          wifi.scan_time_msec
    ...
    bodyRows = bodyRows.concat(
      historian.appstats.getControllerActivityDetails("Wifi", wifi_controller)
    );
    bodyRows = bodyRows.concat(
      historian.appstats.getControllerActivityDetails("Modem", modem_controller)
    );
    bodyRows = bodyRows.concat(
      historian.appstats.getControllerActivityDetails(
        "Bluetooth",
        bt_controller
      )
    );
      ...
      bodyRows.push([
        ...
        goog.string.subs(
          ...
          bt_misc.ble_scan_time_msec
            ? historian.time.formatDuration(bt_misc.ble_scan_time_msec)
            : "0s"
        ),
      ]);
    }
    var table = historian.tables
      .createTable(null, bodyRows)
  ...
  if (
        ...
        bodyRows.push([
          {
            ...
            title:
              "Sleeping is mostly screen off, but also includes the the" +
        ...
        bodyRows.push([
          {
            ...
            title:
              "This does not include time running as top, top sleeping," +
    ...
    if (processes && processes.length > 0) {
      ...
      processes.sort(function (a, b) {
        return (
          b.user_time_msec - a.user_time_msec ||
...
historian.appstats.displayAppSensor = function (sensors) {
  ...
  if (sensors && sensors.length > 0) {
    ...
    for (var i = 0; i < sensors.length; i++) {
      ...
      bodyRows.push([
        name,
        s.TotalTimeMs ? historian.time.formatDuration(s.TotalTimeMs) : "",
        s.Count,
      ]);
    }
    var table = historian.tables.createTable(headRow, bodyRows);
...
historian.appstats.displayAppWakelock = function (wakelocks) {
  ...
  if (wakelocks && wakelocks.length > 0) {
    ...
    wakelocks.sort(function (a, b) {
      return (
        b.full_time_msec - a.full_time_msec ||
    ...
    var partialTimeTitle =
      "Lower bound of the total time held holding a " + "partial wake lock";
    if (historian.reportVersion >= 21) {
    ...
    for (var i = 0; i < wakelocks.length; i++) {
      ...
      if (historian.reportVersion >= 20) {
        var cur = w.partial_current_duration_msec
          ? goog.string.subs(
              ...
              historian.time.formatDuration(w.partial_current_duration_msec)
            )
          : "";
        var maxFormat = w.partial_max_duration_msec
          ...
          : "";
        var ptVal =
          w.partial_max_duration_msec > w.partial_time_msec ? maxFormat : pt;
        if (historian.reportVersion >= 21) {
          // Partial total time is available.
          ptVal = historian.time.formatDuration(
            w.partial_total_duration_msec || 0
          );
        }
        pt = {
          value: ptVal,
          title: goog.string.subs(
            "Longest individual wakelock duration: %s%s",
            maxFormat,
            cur
          ),
        };
      ...
      var row = [
        w.name,
        w.full_time_msec ? historian.time.formatDuration(w.full_time_msec) : "",
        w.full_count,
        pt,
        w.partial_count,
        w.window_time_msec
...
historian.appstats.displayApp = function (appUid) {
  ...
  if (app.RawStats.foreground) {
    bodyRows.push([
      ...
      goog.string.subs(
        ...
        historian.time.formatDuration(app.RawStats.foreground.total_time_msec)
      ),
    ]);
  }
  ...
  if (app.RawStats.audio) {
    bodyRows.push([
      "Audio",
      goog.string.subs(
        "%s times for a total duration of %s",
        app.RawStats.audio.count,
        historian.time.formatDuration(app.RawStats.audio.total_time_msec)
      ),
    ]);
  }
  if (app.RawStats.camera) {
    bodyRows.push([
      ...
      goog.string.subs(
        ...
        historian.time.formatDuration(app.RawStats.camera.total_time_msec)
      ),
    ]);
  }
  if (app.RawStats.flashlight) {
    bodyRows.push([
      ...
      goog.string.subs(
        ...
        historian.time.formatDuration(app.RawStats.flashlight.total_time_msec)
      ),
    ]);
  }
  if (app.RawStats.video) {
    bodyRows.push([
      ...
      goog.string.subs(
        ...
        historian.time.formatDuration(app.RawStats.video.total_time_msec)
      ),
    ]);
  }
  var table = historian.tables
  ...
  historian.appstats.displayAppNetworkInfo(
    app.RawStats.network,
    app.RawStats.wifi,
    app.RawStats.wifi_controller,
    app.RawStats.modem_controller,
    app.RawStats.bluetooth_controller,
    app.RawStats.bluetooth_misc
  );

  historian.appstats.displayAppProcess(
    app.RawStats.process,
...
historian.appstats.sortAppSelector_ = function () {
  ...
  for (var i = 0; i < historian.appstats.appOptions.length; i++) {
    $(historian.appstats.APP_SELECTOR_ID_).append(
      historian.appstats.appOptions[i].option
    );
  }
  if (selected) {
    // Preserve the original selection.
    $(historian.appstats.APP_SELECTOR_ID_).val(
...
historian.appstats.showSortedAppTable_ = function () {
  ...
  for (var i = 0; i < historian.appstats.appOptions.length; i++) {
    ...
    var val = historian.appstats.getValue(
      appOption.stat,
      /** @type {string} */ (selection)
    );
    if (!val) {
